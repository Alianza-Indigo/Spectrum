...
/** POST /api/cases/:id/ai/summarize — resumen asistido (borrador para revisión). */
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await runCaseAi(session, id, "summarize", parsed.data.input, parsed.data.inputDocs);
